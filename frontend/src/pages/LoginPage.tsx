...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await apiFetch<{ accessToken: string; code?: string }>("/auth/login", {
        method: "POST",
      ...
      <div className="w-full max-w-md animate-fade-in-up z-10">
        <div className="glass rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden">
          <div className="relative">
            ...
            {notVerified && (
                <div className="mb-6 rounded-2xl bg-amber-50 border border-amber-200 p-4 shadow-sm animate-fade-in-up">
                  <div className="flex items-start gap-3">
                    <MailWarning className="h-5 w-5 text-amber-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-bold text-amber-900 mb-1">Email not verified</p>
                      <p className="text-sm text-amber-700 mb-3">Please check your inbox and click the verification link before logging in.</p>
                      <button onClick={handleResend} disabled={resending} className="text-xs font-bold text-amber-800 underline hover:text-amber-900 disabled:opacity-50">
                        {resending ? "Sending…" : "Resend verification email"}
                      </button>
                      {resendMsg && <p className="text-xs text-green-700 font-medium mt-1">{resendMsg}</p>}
                    </div>
                  </div>
                </div>
            )}
