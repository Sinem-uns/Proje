...
import { Building2, MapPin, Mail, Lock, User, Hash, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export const RegisterPage = () => {
  ...
  useEffect(() => {
    apiFetch<{ exists: boolean }>("/auth/admin-exists")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch<any>("/auth/register", {
        method: "POST",
        body: JSON.stringify({ first_name: firstName, last_name: lastName, email, role, password, institution, city, country }),
      });
      setSuccess(true);
    } catch (err: any) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <select className="w-full rounded-2xl border border-slate-200 bg-white/50 pl-11 pr-8 py-3 text-sm text-slate-800 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all outline-none appearance-none font-medium" value={role} onChange={(e) => setRole(e.target.value)}>
