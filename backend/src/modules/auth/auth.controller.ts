...
authRouter.post("/register", authLimiter, async (req, res, next) => {
  try {
    ...
    if (parsed.role === "ADMIN") {
      ...
      if (!adminSecret) return res.status(403).json({ error: "Admin registration is not enabled on this server." });
      if (!parsed.admin_secret || parsed.admin_secret !== adminSecret) {
        return res.status(403).json({ error: "Invalid admin registration code. Contact your system administrator." });
      }
    }
